...
import { isIE, isEdge } from '../../src/platforms/web/util'
...
describe('parser', () => {
  ...
  it('returns `template` for multiple root elements', () => {
    const ast = parse('<div></div><div></div>', baseOptions)

    expect(ast.tag).toEqual('template')
  ...
  it('remove text nodes between r-if conditions', () => {
    const ast = parse(`<div><div r-if="1"></div> <div r-else-if="2"></div> <div r-else></div> <span></span></div>`, baseOptions)
    expect(ast.children.length).toBe(3)
  ...
  it('warn non whitespace text between r-if conditions', () => {
    parse(`<div><div r-if="1"></div> foo <div r-else></div></div>`, baseOptions)
    expect(`text "foo" between r-if and r-else(-if) will be ignored`).toHaveBeenWarned()
  })

  it('not warn 2 root elements with r-if and r-else', () => {
    parse('<div r-if="1"></div><div r-else></div>', baseOptions)
    expect('Component template should contain exactly one root element')
      .not.toHaveBeenWarned()
  })

  it('not warn 3 root elements with r-if, r-else-if and r-else', () => {
    parse('<div r-if="1"></div><div r-else-if="2"></div><div r-else></div>', baseOptions)
    expect('Component template should contain exactly one root element')
      .not.toHaveBeenWarned()
  })

  it('not warn 2 root elements with r-if and r-else on separate lines', () => {
    parse(`
      <div r-if="1"></div>
      <div r-else></div>
    `, baseOptions)
  ...
  it('not warn 3 or more root elements with r-if, r-else-if and r-else on separate lines', () => {
    parse(`
      <div r-if="1"></div>
      <div r-else-if="2"></div>
      <div r-else></div>
  ...
  it('generate correct ast for 2 root elements with r-if and r-else on separate lines', () => {
    const ast = parse(`
      <div r-if="1"></div>
      <p r-else></p>
    `, baseOptions)
  ...
  it('generate correct ast for 3 or more root elements with r-if and r-else on separate lines', () => {
    const ast = parse(`
      <div r-if="1"></div>
      <span r-else-if="2"></span>
      <p r-else></p>
    `, baseOptions)
    expect(ast.tag).toBe('div')
  ...
  it('parses <slot> root element as fragment', () => {
    const ast = parse('<slot></slot>', baseOptions)

    expect(ast.tag).toBe('template')
    expect(ast.isFragment).toBe(true)
    expect(ast.children[0].tag).toBe('slot')
    expect(ast.children.length).toBe(1)
  })

  it('parses <template> root element as fragment', () => {
    const ast = parse('<template></template>', baseOptions)

    expect(ast.tag).toBe('template')
    expect(ast.isFragment).toBe(true)
    expect(ast.children[0].tag).toBe('template')
    expect(ast.children.length).toBe(1)
  })

  it('parses r-for on root element into template fragment', () => {
    const ast = parse('<div r-for="item in items"></div>', baseOptions)

    expect(ast.tag).toBe('template')
    expect(ast.isFragment).toBe(true)
    expect(ast.children[0].tag).toBe('div')
    expect(ast.children[0].attrsMap['r-for']).toBe('item in items')
    expect(ast.children.length).toBe(1)
  })

  it('parses multiple root elements into template fragment', () => {
    const ast = parse('<div></div><div></div>', baseOptions)

    expect(ast.tag).toBe('template')
    expect(ast.isFragment).toBe(true)
    expect(ast.children.map(t => t.tag)).toEqual(['div', 'div'])
    expect(ast.children.length).toBe(2)
  })

  it('warn <template> key', () => {
    parse('<div><template r-for="i in 10" :key="i"></template></div>', baseOptions)
    expect('<template> cannot be keyed').toHaveBeenWarned()
  })

  it('r-pre directive', () => {
    const ast = parse('<div r-pre id="message1"><p>{{msg}}</p></div>', baseOptions)
  ...
  it('r-for directive basic syntax', () => {
    const ast = parse('<ul><li r-for="item in items"></li></ul>', baseOptions)
    const liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('item')
  })

  it('r-for directive iteration syntax', () => {
    const ast = parse('<ul><li r-for="(item, index) in items"></li></ul>', baseOptions)
    const liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('item')
  ...
  it('r-for directive iteration syntax (multiple)', () => {
    const ast = parse('<ul><li r-for="(item, key, index) in items"></li></ul>', baseOptions)
    const liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('item')
  ...
  it('r-for directive key', () => {
    const ast = parse('<ul><li r-for="item in items" :key="item.uid"></li></ul>', baseOptions)
    const liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('item')
  ...
  it('r-for directive destructuring', () => {
    let ast = parse('<ul><li r-for="{ foo } in items"></li></ul>', baseOptions)
    let liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('{ foo }')

    // with paren
    ast = parse('<ul><li r-for="({ foo }) in items"></li></ul>', baseOptions)
    liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('{ foo }')

    // multi-var destructuring
    ast = parse('<ul><li r-for="{ foo, bar, baz } in items"></li></ul>', baseOptions)
    liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    ...
    // with index
    ast = parse('<ul><li r-for="({ foo }, i) in items"></li></ul>', baseOptions)
    liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('{ foo }')
    ...
    // with key + index
    ast = parse('<ul><li r-for="({ foo }, i, j) in items"></li></ul>', baseOptions)
    liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('{ foo }')
    expect(liAst.iterator1).toBe('i')
    ...
    ast = parse('<ul><li r-for="({ foo, bar, baz }, i) in items"></li></ul>', baseOptions)
    liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('{ foo, bar, baz }')
    expect(liAst.iterator1).toBe('i')
    ...
    // nested
    ast = parse('<ul><li r-for="({ foo, bar: { baz }, qux: [ n ] }, i, j) in items"></li></ul>', baseOptions)
    liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    ...
    // array nested
    ast = parse('<ul><li r-for="([ foo, { bar }, baz ], i, j) in items"></li></ul>', baseOptions)
    liAst = ast.children[0]
    expect(liAst.for).toBe('items')
    expect(liAst.alias).toBe('[ foo, { bar }, baz ]')
    expect(liAst.iterator1).toBe('i')
  ...
  it('r-for directive invalid syntax', () => {
    parse('<ul><li r-for="item into items"></li></ul>', baseOptions)
    expect('Invalid r-for expression').toHaveBeenWarned()
  })

  it('r-if directive syntax', () => {
    const ast = parse('<p r-if="show">hello world</p>', baseOptions)
    expect(ast.if).toBe('show')
    expect(ast.ifConditions[0].exp).toBe('show')
  })

  it('r-else-if directive syntax', () => {
    const ast = parse('<div><p r-if="show">hello</p><span r-else-if="2">elseif</span><p r-else>world</p></div>', baseOptions)
  ...
  it('r-else directive syntax', () => {
    const ast = parse('<div><p r-if="show">hello</p><p r-else>world</p></div>', baseOptions)
    const ifAst = ast.children[0]
    const conditionsAst = ifAst.ifConditions
  ...
  it('r-else-if directive invalid syntax', () => {
    parse('<div><p r-else-if="1">world</p></div>', baseOptions)
    expect('r-else-if="1" used on element').toHaveBeenWarned()
  })

  it('r-else directive invalid syntax', () => {
    parse('<div><p r-else>world</p></div>', baseOptions)
    expect('r-else used on element').toHaveBeenWarned()
  })

  it('r-once directive syntax', () => {
    const ast = parse('<p r-once>world</p>', baseOptions)
  ...
  it('attribute with directive', () => {
    const ast = parse('<input type="text" name="field1" :value="msg" r-validate:field1="required">', baseOptions)
  ...
  it('attribute with modifiered directive', () => {
    const ast = parse('<input type="text" name="field1" :value="msg" r-validate.on.off>', baseOptions)
  ...
  it('pre/post transforms', () => {
    ...
    parse('<img r-pre src="hi">', options)
