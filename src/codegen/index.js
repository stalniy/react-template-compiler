...
import { camelize, no, extend } from '../util'
  ...
  if (el.if && !el.ifProcessed) {
    ...
    if (!key) {
      process.env.NODE_ENV !== 'production' && state.warn(
        `r-once can only be used inside r-for that is keyed. `
      )
...
function genIfConditions (
  ...
  // r-if with r-once should generate code like (a)?_m(0):_m(1)
...
export function genFor (
  ...
  if (process.env.NODE_ENV !== 'production' &&
    ...
    state.warn(
      `<${el.tag} r-for="${alias} in ${exp}">: component lists rendered with ` +
      `r-for should have explicit keys. ` +
