  ...
  if (node.type === 1) {
    for (const name in node.attrsMap) {
      if (dirRE.test(name)) {
        ...
        if (value) {
          if (name === 'r-for') {
            checkFor(node, `r-for="${value}"`, errors)
...
function checkFor (node: ASTElement, text: string, errors: Array<string>) {
  ...
  checkIdentifier(node.alias, 'r-for alias', text, errors)
  checkIdentifier(node.iterator1, 'r-for iterator', text, errors)
  checkIdentifier(node.iterator2, 'r-for iterator', text, errors)
}
